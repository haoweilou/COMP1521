...
int physicalAddress (uint vAddr, char action)
{
	...
	uint pAddr = 0;
	//return -1 if page is not valid
	if (page < 0)  return -1;
	if (page > nPages-1) return -1;
	//if the page is already loaded,update time and return pAddr
	if(PageTable[page].frameNo != -1){
		if(action == 'W') PageTable[page].status.modified = 1;
		PageTable[page].lastAccessed = clock;
		pAddr = PageTable[page].frameNo*PAGESIZE + offset;
		return pAddr;
	} 
	
	//go_frame is used to store the frame number that page is going to load
	int go_frame = -1;
	...
	//if can't find an unused frame
	if(go_frame == -1){
		uint leastpage = 0;
		//find the least recently loaded page(smallest t number)
		for(int i = 0; i < nPages;){
			if(PageTable[leastpage].lastAccessed == -1){
				leastpage++;
				continue;
			}
			if(PageTable[i].lastAccessed < PageTable[leastpage].lastAccessed 
			&& PageTable[i].lastAccessed != -1){
				leastpage = i;
			}
			i++;
		}
		//set desti frame be the least recently used page's frame
		go_frame = PageTable[leastpage].frameNo;
		//empty the least recently used page 
		if(PageTable[leastpage].status.modified == 1) nSaves++;
		PageTable[leastpage].frameNo = -1;
		PageTable[leastpage].lastAccessed = -1;
		PageTable[leastpage].status.modified = 0;
		PageTable[leastpage].status.loaded = 0;
		//empty the desti memory
		MemFrames[go_frame] = -1;
		nReplaces++;
	}

	if(action == 'W') PageTable[page].status.modified = 1;
	if(action == 'W') PageTable[page].status.loaded = 1;
	if(action == 'R') PageTable[page].status.loaded = 1;
	nLoads++;

	MemFrames[go_frame] = page;
	PageTable[page].frameNo = go_frame;
	PageTable[page].lastAccessed = clock;
	pAddr = go_frame*PAGESIZE + offset;
